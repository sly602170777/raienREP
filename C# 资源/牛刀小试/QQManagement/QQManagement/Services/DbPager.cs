...
namespace QQManagement.Services
{
    ...
    public class DbPagerBase
    {
        ...
        internal virtual void CreateRecordCountSQL()
        {
            if (dbPager.isDefaultRecordCount)
            {
                ...
                {
                    dbPager.pagerCountSql = String.Format(
                        "SELECT COUNT(0) FROM {0} {1}",
                        dbPager.tableName,
                        dbPager.appendSql
                    );
                }
                else
                {
                    dbPager.pagerCountSql = String.Format(
                        "SELECT COUNT(0) FROM {0} {1}",
                        dbPager.tableName,
                        string.IsNullOrEmpty(dbPager.whereSql) ? "" : "WHERE " + dbPager.whereSql
                    );
                }
            }
            else
            {
                ...
                {
                    dbPager.pagerCountSql = String.Format(
                        "SELECT COUNT(0) FROM (SELECT {0} FROM {1} {2}) AS [TABLE]",
                        dbPager.field,
                        dbPager.tableName,
                        dbPager.appendSql
                    );
    ...
    public class SQL_Max : DbPagerBase
    {
        internal override void CreateNextPageSQL()
        {
            ...
            string orderCol;
            bool isDesc = true;
            orderCol = dbPager.orderSql.ToLower();
    ...
    public class SQL_TopTop : DbPagerBase
    {
        ...
        private void TopTopInit(string t1, string t2)
        {
            ...
            {
                _tableNames = dbPager.tableName;
                _query = dbPager.whereSql;
            ...
            if (!_orderCol.Contains(_idColumn))
                _orderCol += "," + _idColumn;
        }
        ...
        internal override string GetSQLByPageIndex()
        {
            ...
            int pagerlastcount =
                dbPager.pagerCount % dbPager.pagerSize == 0
                    ? dbPager.pagerCount / dbPager.pagerSize
                    : dbPager.pagerCount / dbPager.pagerSize + 1;
