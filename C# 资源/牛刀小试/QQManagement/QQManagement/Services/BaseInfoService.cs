...
namespace QQManagement.Services
{
    public class BaseInfoService
    {
        ...
        )
        {
            ...
            var sql =
                $"insert into [dbo].[BaseInfo] values({qqUser.QQId},'{nickName}','{sex}','{born}','{city}',{phone})";
            DBHelp.Update(sql);
        ...
        {
            var sql =
                $"select QQId, NickName, Sex, BornDate, City, Phone from [dbo].[BaseInfo] where QQId = {qqid}";
        ...
        {
            var sql =
                $"select [QQId], [NickName], [Sex], [BornDate], [City], [Phone] from [dbo].[BaseInfo]";
        ...
        {
            var sql =
                $"select [QQId], [NickName], [Sex], [BornDate], [City], [Phone] from [dbo].[BaseInfo] where NickName like '%{word}%'";
            List<BaseInfo> baseInfos = new List<BaseInfo>();
            var dataRead = DBHelp.SelectForReader(sql);
            try
            {
                ...
                {
                    Debug.WriteLine(
                        $"QQId: {dataRead.GetInt32(0)}, NickName: {dataRead.GetString(1)}, Sex: {dataRead.GetString(2)}, BornDate: {dataRead.GetDateTime(3)}, City: {dataRead.GetString(4)}, Phone: {dataRead.GetInt32(5)}"
                    );

                    Debug.WriteLine(
                        $"QQId: {dataRead.GetInt32(0)}, Type: {dataRead.GetInt32(0).GetType()}"
                    );
                    Debug.WriteLine(
                        $"NickName: {dataRead.GetString(1)}, Type: {dataRead.GetString(1).GetType()}"
                    );
                    Debug.WriteLine(
                        $"Sex: {dataRead.GetString(2)}, Type: {dataRead.GetString(2).GetType()}"
                    );
                    Debug.WriteLine(
                        $"BornDate: {dataRead.GetDateTime(3)}, Type: {dataRead.GetDateTime(3).GetType()}"
                    );
                    Debug.WriteLine(
                        $"City: {dataRead.GetString(4)}, Type: {dataRead.GetString(4).GetType()}"
                    );
                    Debug.WriteLine(
                        $"Phone: {dataRead.GetInt32(5)}, Type: {dataRead.GetInt32(5).GetType()}"
                    );
                    baseInfos.Add(
                        new BaseInfo(
                            (long)dataRead.GetInt32(0),
            ...
            catch (InvalidCastException ex)
            {
                ...
                Debug.WriteLine($"InvalidCastException: {ex.Message}");
                Debug.WriteLine($"Error Code: {ex.HResult}");
